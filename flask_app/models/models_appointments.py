...
class Appointment:
    ...
    @classmethod
    def update_unconfirmed_appts_provider(cls, data):
        query = "UPDATE appointments SET first_name = %(first_name)s, last_name = %(last_name)s, seeker_email = %(seeker_email)s, seeker_phone_number = %(seeker_phone_number)s, apptdate = %(apptdate)s, appttime = %(appttime)s, updated_at = NOW() WHERE id = %(id)s"
        return connectToMySQL('helping_hand_schema').query_db(query,data)
    ...
    def show_confirmed_appts_seeker(cls, data):
        query = "SELECT * FROM users LEFT JOIN appointments ON users.email = appointments.user_email LEFT JOIN work_address ON appointments.user_email = work_address.user_email WHERE appointments.confirmed = 1 AND appointments.requested = 0 AND appointments.seeker_email = %(email)s"
        return connectToMySQL('helping_hand_schema').query_db(query, data)
    ...
    def show_requested_appts_seeker(cls, data):
        query = "SELECT * FROM users LEFT JOIN appointments ON users.email = appointments.user_email WHERE appointments.confirmed = 0 AND appointments.requested = 1 AND appointments.seeker_email = %(email)s;"
        return connectToMySQL('helping_hand_schema').query_db(query, data)

    @classmethod
    def show_unconfirmed_appts_seeker(cls, data):
        query = "SELECT * FROM users LEFT JOIN appointments ON users.email = appointments.user_email JOIN work_address ON appointments.user_email = work_address.user_email WHERE appointments.confirmed = 0 AND appointments.requested = 0 AND appointments.seeker_email = %(email)s;"
        return connectToMySQL('helping_hand_schema').query_db(query,data)
    
    @classmethod
    def confirm_unconfirmed_appts_seeker(cls, data):
        query = "UPDATE appointments SET confirmed = %(confirmed)s, updated_at = NOW() WHERE id = %(id)s"
        return connectToMySQL('helping_hand_schema').query_db(query,data)
